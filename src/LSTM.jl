...
# ╔═╡ 727a2cc6-004d-11ef-1a70-b78272f53cc8
begin
...
# ╔═╡ 09d466a1-f250-47ea-8a6c-69a14c8d738e
begin
	fns = ["Data_S1.csv", "Data_S2.csv", "Data_S3.csv", "Data_S4.csv"]
	data = [DataFrame(CSV.File("data/$fn")) for fn in fns]
	data = vcat(data...)
	data.DATE_TIME = DateTime.(data.DATE_TIME, "Y-m-d HH:MM:SS")
	irradiance_cols = [:POA]

	test = DataFrame(CSV.File("data/Test_S4.csv"))
	test.DATE_TIME = DateTime.(test.DATE_TIME, "m/d/Y HH:MM")
	first_forecast,last_forecast = extrema(test.DATE_TIME)
	for name in names(data)[2:end]
		test[!, name] .= NaN
	end
	...
	data.rounded_time = round.(data.DATE_TIME, Hour(1))
	combined_data = leftjoin(data, weather, on=:rounded_time=>:datetime)
	unique!(combined_data)
	
	combined_data.hour_dec = hour.(combined_data.DATE_TIME) + minute.(combined_data.DATE_TIME) / 60

	combined_data.neg_cos_time = (1/2) * (-cos.(combined_data.hour_dec * 2π/24) .+ 1)
	
	pushfirst!(weather_cols, :neg_cos_time)

	select!(combined_data, :DATE_TIME, weather_cols, irradiance_cols)
	
	combined_data[!, 2:end] = Float32.(combined_data[:, 2:end])

	train_df = combined_data[1:findlast(dt -> dt < first_forecast, combined_data.DATE_TIME)-1, :]
end
...
transforms = Dict(
    names(train_df)[i] => fit(UnitRangeTransform, train_df[:, i])
    for i in 3:size(train_df, 2)
)

# ╔═╡ c3fd9501-67ed-43ce-8da2-b80ad0c20e33
begin
	batch_size = 2^5
	X = train_df[train_index, [weather_cols; irradiance_cols]]

	
	for col in names(X)[2:end]
		X[!, col] = StatsBase.transform(transforms[col], X[:, col])
	end
	
	X = X |> Matrix |> transpose
	X = [X[:, i:i-1+batch_size] for i in 1:size(X,2)-(batch_size-1)]
	Y = [x[end-length(irradiance_cols)+1:end, :] for x in X]
	Train = [(past=x,next=y) for (x,y) in zip(X[1:end-1], Y[2:end])]

	X = train_df[test_index, [weather_cols; irradiance_cols]]
	for col in names(X)[2:end]
		X[!, col] = StatsBase.transform(transforms[col], X[:, col])
	end
	X =  X |> Matrix |> transpose
...
begin
	hidden_dims = 2^3
	model = Chain(
		LSTM_in = LSTM(size(first(X), 1)=>hidden_dims),
		Dense_Out = Dense(hidden_dims => length(irradiance_cols), σ)
	)
...
begin
	for e in 1:2^3
		...
		for t in Train[2:end]
			train!(loss, model, t, opt_state; train_log=temp_train_log)
		end
		push!(train_loss, mean(temp_train_log))
...
begin
	Flux.reset!(model)
	[model(x.past) for x in Train[1:end-1]]
	results = model(last(Train).past) |> transpose
	plot(
		results, label="Ŷ", color=:green, lw=4, α=0.5
	)
	
	plot!(
		Train[end-1].next |> transpose,
		label="Y", color=:darkgreen, markerstrokewidth=0.0, ms=3, marker=:o
	)
end
...
function forecast(data, firstforecast, lastforecast)
	firstforecast = findlast(row -> row ≤ firstforecast, data.DATE_TIME)
	lastforecast = findlast(row -> row <= lastforecast, data.DATE_TIME)
	
	xfrm = deepcopy(data[:, [weather_cols; irradiance_cols]])

	for col in names(xfrm)[2:end]
		xfrm[!, col] = StatsBase.transform(transforms[col], xfrm[:, col])
	end
	...
	StatsBase.reconstruct!(transforms["POA"], results)
	...
	out_df.POA_pred[firstforecast:lastforecast] = results
	#xfrm[firstforecast:lastforecast, :POA_pred, ] = out_df |> transpose
	out_df
...
manifest_format = "2.0"
project_hash = "ff2a6a131fde26f6602040117cb3f574bfa63111"
