...
We partition the data by recieved datasets and according to a train/test split which used 80% of the data for training and reserves 20% for evaluating the models on unseen data.

```{r, train-test-split, echo=FALSE}
# create a train and test index
train_test_split <- 4/5
s1 <- 1:8859
s1_train <- 1:floor(last(s1) * train_test_split)
s1_test <- 1+last(s1_train):last(s1)

s2 <- 1:9001
s2_train <- 1:floor(last(s2) * train_test_split)
s2_test <- 1+last(s2_train):last(s2)
```
...
The models used are detailed below. The numbers in parenthesis after the model indicate the testing period(s) they were considered for and a \* indicates it was the selected model for that period.

```{r}
POA_TS_s1 <- ts(data$POA[s1_train], frequency = 6*24)
```
...
```{r, seasonal-naive, echo=FALSE}
POA_snaive <- snaive(POA_TS, h = 250)
autoplot(POA_snaive)
```
```{r}
POA_snaive$fitted
...
naive <- function(date_time[s1_train], data = data) {
...
## Seasonal Mean
There is clearly no reason to explore a "mean" forecast due to the obvious daily cycle. The seasonal mean, however, does seem to offer a reasonable forecast.
...
## Error Quantification and Model Training Performance
